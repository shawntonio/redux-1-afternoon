...
import store, {UPDATE_INSTRUCTIONS, UPDATE_RECIPES, RESET_RECIPE} from '../../store';
...
class Instructions extends Component {
  ...
  addInstruction() {
    store.dispatch({type: UPDATE_INSTRUCTIONS, payload: this.state.input})
    this.setState({
      input: ""
    });
  }
  create() {
    store.dispatch({type: UPDATE_RECIPES})
    store.dispatch({type: RESET_RECIPE})
  }
  render() {
